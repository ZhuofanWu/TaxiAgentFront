...
const BackIcon = {
  render: () => h('svg', { viewBox: '0 0 512 512', fill: 'currentColor', style: 'width: 1em; height: 1em;' }, [
    h('path', { d: 'M177.9 273.1L96 359.2l-9.8-86.1H32c-17.7 0-32-14.3-32-32s14.3-32 32-32h54.3l9.8-86.1 81.9 86.1c12.4 12.4 12.4 32.5 0 44.9z' }),
  ]),
}
  ...
  <div class="support-ticket-detail">
            ...
            <NCard size="small" class="action-card">
              <div class="action-buttons">
                <template v-if="!isAssigned">
                  <NButton type="warning" @click="handleClaim">
                    认领
                  </NButton>
                </template>
                <template v-else-if="isHandledByCurrentUser">
                  <NButton v-if="canEscalate" type="error" @click="openEscalateModal">
                    加急
                  </NButton>
                  <NButton type="warning" @click="openTransferModal">
                    转交
                  </NButton>
                  <NButton type="error" @click="openRejectModal">
                    驳回
                  </NButton>
    ...
    <NModal v-model:show="showRejectModal" preset="card" title="驳回工单" style="width: 400px">
      ...
      <template #action>
        ...
        <NButton type="error" :loading="rejectLoading" @click="handleReject">
          确认驳回
        </NButton>
      </template>
    </NModal>
